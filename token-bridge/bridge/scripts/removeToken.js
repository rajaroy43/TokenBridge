...
module.exports = async (callback) => {
  try {
    ...
    if (!tokenToRemove) console.error("You need to pass the token address");
    ...
    console.log("allowTokenContractAddress", allowTokensAddress);
    ...
    const tokenToRemoveData = allowTokens.contract.methods.removeAllowedToken(tokenToRemove).encodeABI();

    console.log(tokenToRemoveData);
    const result = await multiSig.methods
      .submitTransaction(allowTokens.address, 0, tokenToRemoveData)
