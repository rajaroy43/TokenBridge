...
Befor run `integrationTest` ,go to ../bridge/ and run `npm run deployIntegrationTest` . It will deploy all contract related to bridge on both local ganache .
...
```json
(module.exports = {
  "mainchain": require("./development.json"),
  "sidechain": require("./development2.json"),
  "runEvery": 1,
  "confirmations": 10,
...
where the mainchain for example is 1st blockchain and the sidechain is 2nd blockchain, the .json files are in the /federator/config folder and includes the addresses of the contracts in that network and the block number when they where deployed.
