...
contract("AllowTokens", async function(accounts) {
  ...
  describe("Tokens whitelist", async function() {
    ...
    it("before enables tokens to whitelist", async function() {
      let isValidatingAllowedTokens = await this.allowTokens.isValidatingAllowedTokens();
      assert.equal(isValidatingAllowedTokens, true);
      let isTokenAllowed = await this.allowTokens.isTokenAllowed(this.token.address);
    ...
    it("Tokens validation and there is no whitelist", async function() {
      await this.allowTokens.disableAllowedTokensValidation({ from: manager });
      let isValidatingAllowedTokens = await this.allowTokens.isValidatingAllowedTokens();
      assert.equal(isValidatingAllowedTokens, false);
      let isTokenAllowed = await this.allowTokens.isTokenAllowed(this.token.address);
      assert.equal(isTokenAllowed, true);
    });
  ...
  describe("Max tokens allowed", async function() {
    ...
    it("sets a new amount of max tokens", async function() {
      let newMaxTokens = web3.utils.toWei("500000");
    ...
    it("fail if not the owner", async function() {
      ...
      utils.expectThrow(this.allowTokens.setMaxTokensAllowed(web3.utils.toWei("500000"), { from: tokenDeployer }));
  ...
  describe("Fee And Min Per token", async function() {
    ...
    it("fail if not the owner", async function() {
      ...
      utils.expectThrow(this.allowTokens.setFeeAndMinPerToken(this.token.address, newFee, newMin, { from: tokenDeployer }));
    ...
    it("fail if min is bigger than max", async function() {
      ...
      utils.expectThrow(
        this.allowTokens.setFeeAndMinPerToken(this.token.address, web3.utils.toWei("2"), web3.utils.toWei("11"), { from: manager })
      );
    });

    it("fail if fee is bigger than min", async function() {
      utils.expectThrow(
        this.allowTokens.setFeeAndMinPerToken(this.token.address, web3.utils.toWei("2"), web3.utils.toWei("1"), { from: manager })
      );
    });

    it("fail if fee is 0", async function() {
  ...
  describe("Max Daily Limit tokens", async function() {
    it("should set initial values", async function() {
      ...
      assert.equal(dailyLimit, web3.utils.toWei("1000000"));
    });

    it("change daily limit", async function() {
      let newDailyLimit = web3.utils.toWei("500000");
    ...
    it("fail if not the owner", async function() {
      ...
      utils.expectThrow(this.allowTokens.changeDailyLimit(web3.utils.toWei("500000"), { from: tokenDeployer }));
  ...
  describe("isValidTokenTransfer", async function() {
    ...
    it("should check min value", async function() {
      await this.allowTokens.setFeeAndMinPerToken(this.token.address, web3.utils.toWei("3"), web3.utils.toWei("5"), { from: manager });
    ...
    it("should check daily limit", async function() {
      ...
      let result = await this.allowTokens.isValidTokenTransfer(
        this.token.address,
        minTokensAllowed,
        new BN(dailyLimit).sub(new BN(minTokensAllowed)),
        true
      );
      assert.equal(result, true);
    ...
    it("should check feeConstToken per token is set", async function() {
      let maxTokensAllowed = await this.allowTokens.getMaxTokensAllowed();
      let result = await this.allowTokens.isValidTokenTransfer(this.token.address, maxTokensAllowed, 0, true);
      assert.equal(result, false);
  ...
  describe("Calls from MultiSig", async function() {
    ...
    it("should fail to add a new allowed token due to less  confirmations", async function() {
    ...
    it("should fail to remove an allowed token due to less confirmations", async function() {
      let data = this.allowTokens.contract.methods.addAllowedToken(this.token.address).encodeABI();
      await this.multiSig.submitTransaction(this.allowTokens.address, 0, data, { from: multiSigOnwerA });
      await this.multiSig.confirmTransaction(0, { from: multiSigOnwerB });
    ...
    it("should fail to disable tokens validation due to less confirmation", async function() {
    ...
    it("should fail to set max tokens due to less confirmations", async function() {
      let maxTokens = await this.allowTokens.getMaxTokensAllowed();
    ...
    it("should set fee and min per token", async function() {
      let newMin = web3.utils.toWei("10");

      let data = this.allowTokens.contract.methods.setFeeAndMinPerToken(this.token.address, newMin, newMin).encodeABI();
      await this.multiSig.submitTransaction(this.allowTokens.address, 0, data, { from: multiSigOnwerA });
      await this.multiSig.confirmTransaction(0, { from: multiSigOnwerB });
