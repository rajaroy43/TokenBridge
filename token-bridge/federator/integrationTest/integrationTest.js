...
async function transfer(originFederators, destinationFederators, config, origin, destination) {
  try {
    ...
    //Send 1 ether on both chain(ganache1 and ganache2) for newly created user
    ...
    data = destinationTokenContract.methods.approve(destinationBridgeAddress, senderBalanceBefore).encodeABI();
    ...
    //run truffle exec .\scripts\setFeeAndMinPerToken.js  --network  development2  destinationTokenAddress  FEE(in 10**18)  minAmount (in 10**18) for setting fee and min per token
    //Setting bsMAIN token fee for destination  chain .
    let multiSigContractForSettingFee = new destinationWeb3.eth.Contract(abiMultiSig, config.sidechain.multiSig);
    let allowTokens = new destinationWeb3.eth.Contract(abiAllowTokens, config.sidechain.allowTokens);
    let fee = 10000000000; //1e10/1e18
    let setFeeAndMinPerTokenData = allowTokens.methods.setFeeAndMinPerToken(destinationTokenAddress, fee, fee).encodeABI();
    let multiSigData = multiSigContractForSettingFee.methods
      .submitTransaction(allowTokens.options.address, 0, setFeeAndMinPerTokenData)
      .encodeABI();
    ...
    data = destinationBridgeContract.methods.receiveTokens(destinationTokenAddress, senderBalanceBefore).encodeABI();
    ...
    //Setting fee for new token
    fee = 10000000000; //1e10/1e18
    setFeeAndMinPerTokenData = allowTokensContract.methods.setFeeAndMinPerToken(anotherTokenAddress, fee, fee).encodeABI();
    multiSigData = multiSigContract.methods.submitTransaction(allowTokensAddress, 0, setFeeAndMinPerTokenData).encodeABI();
    ...
    logger.debug("Check balance on the other side");
    destinationTokenAddress = await destinationBridgeContract.methods.mappedTokens(anotherTokenAddress).call();
    logger.info(`${destination} token address`, destinationTokenAddress);

    logger.debug("Check balance on the other side");
    destinationTokenContract = new destinationWeb3.eth.Contract(abiSideToken, destinationTokenAddress);
    ...
    //setting fee for sidetoken at destination chain
    multiSigContractForSettingFee = new destinationWeb3.eth.Contract(abiMultiSig, config.sidechain.multiSig);
    allowTokens = new destinationWeb3.eth.Contract(abiAllowTokens, config.sidechain.allowTokens);
    fee = 10000000000; //1e10/1e18
    setFeeAndMinPerTokenData = allowTokens.methods.setFeeAndMinPerToken(destinationTokenAddress, fee, fee).encodeABI();
    multiSigData = multiSigContractForSettingFee.methods
      .submitTransaction(allowTokens.options.address, 0, setFeeAndMinPerTokenData)
      .encodeABI();
    await destinationTransactionSender.sendTransaction(config.sidechain.multiSig, multiSigData, 0, "");

    data = destinationTokenContract.methods.send(destinationBridgeAddress, balance, "0x").encodeABI();
    await destinationTransactionSender.sendTransaction(destinationTokenAddress, data, 0, userPrivateKey);
