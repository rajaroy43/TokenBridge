...
module.exports = class Federator {
  ...
  async run() {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        this.logger.info("Running from Block", fromBlock);
        ...
        if (attempt === maxAttempts) {
          ...
          await this.chatBot.sendMessage(`Error running federator after ${maxAttempts} attempts: ${truncatedError}`);
          return;
        } else {
  ...
  async _processLog(log, from) {
    ...
    let transactionId = await this.federationContract.methods
      .getTransactionId(tokenAddress, receiver, amount, symbol, log.blockHash, log.transactionHash, log.logIndex, decimals, granularity)
      .call();
  ...
  async _voteTransaction(tokenAddress, receiver, amount, symbol, blockHash, transactionHash, logIndex, decimals, granularity, userData) {
    ...
    try {
      ...
      txId = await this.federationContract.methods
        .getTransactionId(tokenAddress, receiver, amount, symbol, blockHash, transactionHash, logIndex, decimals, granularity)
        .call();

      if (this.failingTxIds.contains(txId)) {
        this.logger.info(`Transaction with id ${txId}, hash: ${transactionHash} is marked as failing -- not voting for it`);
        return false;
      }
      ...
      if (userData) {
        txData = await this.federationContract.methods
          .voteTransactionAt(tokenAddress, receiver, amount, symbol, blockHash, transactionHash, logIndex, decimals, granularity, userData)
          .encodeABI();
      } else {
        txData = await this.federationContract.methods
          .voteTransaction(tokenAddress, receiver, amount, symbol, blockHash, transactionHash, logIndex, decimals, granularity)
          .encodeABI();
      }
      ...
      this.logger.info(
        `Voted transaction:${transactionHash} of block: ${blockHash} token ${symbol} to Federation Contract with TransactionId:${txId}`
      );
      return true;
    } catch (err) {
      if (txId && this._isEVMRevert(err)) {
        ...
        // queue of those
        const msgHeader =
          `EVM revert when voting for tx with ` + `id: ${txId} hash: ${transactionHash} block: ${blockHash} token: ${symbol}.`;
