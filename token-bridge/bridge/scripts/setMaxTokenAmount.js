...
module.exports = async (callback) => {
  try {
    ...
    const setMaxTokensAllowedData = allowTokens.contract.methods.setMaxTokensAllowed(maximumTokenAmount).encodeABI();

    console.log(`Setting max tokens allowed in ${maximumTokenAmount}`);
    const result = await multiSig.methods
      .submitTransaction(allowTokens.address, 0, setMaxTokensAllowedData)
