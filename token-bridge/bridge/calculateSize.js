...
async function printSize() {
  ...
  files.forEach((item) => {
    ...
    if (sizeInKB > 24) dataTable.push([chalk.red(path.basename(item, ".json")), chalk.red(sizeInKB)]);
    else if (sizeInKB > 20) dataTable.push([chalk.yellow(path.basename(item, ".json")), chalk.yellow(sizeInKB)]);
    else dataTable.push([chalk.green(path.basename(item, ".json")), chalk.green(sizeInKB)]);
  });
