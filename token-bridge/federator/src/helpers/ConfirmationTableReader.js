...
class ConfirmationTableReader {
  constructor(chainId, _confirmationTable = defaultConfirmationTable) {
    ...
    if (!Object.keys(_confirmationTable).includes(chainId.toString())) throw new Error("Invalid chainId");
  ...
  getConfirmations(token, amount) {
    ...
    if (!confirmationArray) return this.confirmationTable.default;
  ...
  _validateTable(confirmationTable) {
    for (const [chainId, table] of Object.entries(confirmationTable)) {
      if (parseInt(chainId) < 0) throw new Error(`ChainId (${chainId}) must be positive`);
      if (parseInt(chainId) != 33 && parseInt(chainId) != 5777) {
        if (!table.default) throw new Error("The table must have a default value");
      }
      const minConfirmation = table.minConfirmation;
      if (parseInt(chainId) != 33 && parseInt(chainId) != 5777) {
        if (!minConfirmation) throw new Error(`minConfirmation is 0 or not defined for chainId ${chainId}`);
      }
      delete table.minConfirmation;
      delete table.default;

      for (const [, confirmationArray] of Object.entries(table)) {
        const arrayMin = confirmationArray.reduce((prev, curr) => (prev.confirmations < curr.confirmations ? prev : curr));
