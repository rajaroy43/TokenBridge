...
contract("Federation", async function(accounts) {
	...
	it("should fail if bigger max memeber length(51)", async function() {
		let members = [];
		for (let i = 0; i <= 50; i++) {
	...
	it("should be successful with max memeber length(50)", async function() {
		let members = [];
		for (let i = 0; i < 50; i++) {
			members[i] = randomHex(20);
		}
	...
	describe("Members", async function() {
		...
		describe("removeMember", async function() {
			...
			it("should fail when removing all members", async function() {
				...
				//Now only 1 federation member is remaining ,if we remove it also then we have 0 member and it fail
	...
	describe("Transactions", async function() {
		...
		beforeEach(async function() {
			...
			this.proxy = await this.project.createProxy(Bridge, {
				initMethod: "initialize",
				initArgs: [deployer, this.federation.address, this.allowTokens.address, this.sideTokenFactory.address, "e"],
			});
		...
		it("voteTransaction should be successful with 1/1 feds require 1", async function() {
			try {
				...
				let transactionId = await this.federation.getTransactionId(
				...
				let transactionWasProcessed = await this.federation.transactionWasProcessed(transactionId);
		...
		it("voteTransaction should fail with wrong acceptTransfer arguments(receiver can't be null)", async function() {
			this.federation.removeMember(fedMember2);
		...
		it("revoke transaction", async function() {
			...
			let txCount = await this.federation.getTransactionCount(transactionId);
			assert.equal(txCount, 0, "Tx count is not zero");
			let txWasProcessed = await this.federation.transactionWasProcessed(transactionId);
			assert.equal(txWasProcessed, false);
			let receipt = await this.federation.voteTransaction(
				originalTokenAddress,
				anAccount,
				amount,
			...
			let hasVoted = await this.federation.hasVoted(transactionId, { from: fedMember1 });
			assert.equal(hasVoted, true);
			txCount = await this.federation.getTransactionCount(transactionId);
			...
			transactionWasProcessed = await this.federation.transactionWasProcessed(transactionId);
			assert.equal(transactionWasProcessed, true);

			let bridgeTxId = await this.bridge.getTransactionId(blockHash, transactionHash, anAccount, amount, logIndex);
			let bridgeTxProcessed = await this.bridge.processed(bridgeTxId);
			assert.equal(bridgeTxProcessed, true);
			...
			await this.federation.transferOwnership(this.multiSig.address);
			let data = this.federation.contract.methods.setRevokeTransactionAndVote(transactionId).encodeABI();
			...
			hasVoted = await this.federation.hasVoted(transactionId, { from: fedMember1 });
			assert.equal(hasVoted, false);
			hasVoted = await this.federation.hasVoted(transactionId, { from: fedMember2 });
			assert.equal(hasVoted, false);
			transactionWasProcessed = await this.federation.transactionWasProcessed(transactionId);
